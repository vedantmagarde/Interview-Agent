...
`
        ...
        const interview = await Interview.create({
            ...
            questions: questionArray.map((que, idx) => ({
                question: que,
                difficulty: ["easy", "medium", "medium", "medium-hard", "hard"][idx],
                timeLimit: [60, 90, 90, 100, 120][idx]
            }))
        })
...
export const submitAnswer = async (req, res) => {
    try {
        ...
        if (!answer) {
            ...
            question.score = 0;
            question.feedback = "Did not submit an answer."
            question.answer = "";
...
export const finishInterview = async (req, res) => {
    try {
        ...
        let totalCorrectess = 0;

        interview.questions.forEach((que) => {
            totalScore += que.score || 0;
            totalConfidence = que.confidence || 0;
            totalCommunication += que.communication || 0;
            totalCorrectess += que.correctness || 0;
        });
        ...
        const avgCorrectness = totalQuestions ? totalCorrectess / totalQuestions : 0;
        ...
        return requestAnimationFrame.status(200).json({
            ...
            questionWiseScore: interview.questions.map((ques) => ({
                question: ques.question,
                score: ques.score || 0,
                feedback: ques.feedback || "",
                confidence: ques.confidence || 0,
                communication: ques.communication || 0,
...
// all interviews for given current user
...
// details for the selected interview for the current user
